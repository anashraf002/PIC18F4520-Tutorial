...
;First we wish to do the high cycle where RA3 
; and RC3 are both high for 0.3 ms or three time units

bsf PORTA, RA3	;set them to be high
bsf PORTC, RC3	;set them to be high
;we want to wait for a total of 0.3 ms
MOVLF 3, ALIVECNT 		;set number of time units (0.1 ms) you want to be HIGH
rcall Delay



; now we clear them both so they're LOW for 0.5 ms
	bcf PORTA, RA3
	bcf PORTC, RC3

MOVLF 5, ALIVECNT 		;set number of time units (0.1 ms) you want to be LOW
rcall Delay
...
;;;;;HIGH TIMER SUBROUTINE;;;;;;;;;;;;;;;;;
Delay
	btg PORTC, RC2		;toggle RC2 so as to reflect a time unit 
	rcall LoopTime	;wait 0.1 ms
	decf ALIVECNT, F ;decrement loop counter and return if not zero
	bnz Delay		; if ALIVECNT not zero then repeat this subroutine otherwise end the delay subroutine
	return	
